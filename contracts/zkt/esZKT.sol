    ...
    AccessControlUpgradeable
{
    ...
    function changeRedemptionStatus(
        bool isActive
    ...
    function startRedemption(uint256 amount, uint256 duration) public {
        ...
        require(
            duration == 15 days || duration == 90 days || duration == 180 days,
            "Invalid duration"
        );

        _transfer(msg.sender, address(this), amount);

        _redemptionRequests[msg.sender].push(
            RedemptionRequest(amount, block.timestamp, duration, false)
        );
        emit RedemptionStarted(
            msg.sender,
            _redemptionRequests[msg.sender].length - 1
        );
    }

    function cancelRedemption(uint256 index) public {
        require(_redemptionActive, "Redemption is currently inactive");
        RedemptionRequest storage request = _redemptionRequests[msg.sender][
            index
        ];
    ...
    function completeRedemption(uint256 index) public {
        ...
        require(!request.completed, "Redemption already completed");
        require(
            block.timestamp >= request.startTime + request.duration,
            "Redemption period not yet over"
        );
     ...
     */
    function getRedemptionRequestCount(
        address account
