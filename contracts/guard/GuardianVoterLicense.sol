    ...
    AccessControlUpgradeable
{
    ...
    function removePromoCode(
        ...
        require(
            _promoCodes[_promoCode].recipient != address(0),
    ...
    function mint(uint256 _amount, string calldata _promoCode) public payable {
        ...
        (bool sent, bytes memory data) = fundsReceiver.call{
            value: finalPrice - referralReward
        }("");
    ...
    function claimReferralReward() external {
        require(
            claimable,
            "Claiming of referral rewards is currently disabled"
        );
    ...
    function setReferralPercentages(
        ...
        require(
            _referralDiscountPercentage <= 99,
            "Referral discount percentage cannot be greater than 99"
        );
        require(
            _referralRewardPercentage <= 99,
            "Referral reward percentage cannot be greater than 99"
        );
        referralDiscountPercentage = _referralDiscountPercentage;
        referralRewardPercentage = _referralRewardPercentage;
        emit ReferralRewardPercentagesChanged(
            _referralDiscountPercentage,
    ...
    function refundNodeLicense(
        ...
        require(
            _exists(_tokenId),
            "ERC721Metadata: Refund for nonexistent token"
        );
