...
contract GuardianReferee is Initializable, AccessControlEnumerableUpgradeable {
    ...
    address public guardianNodeLicenseAddress;

    address public esZKTAddress;
    address public zktAddress;
    ...
    struct Challenge {
        ...
        uint256 rewardAmountForClaimers; // this is how much esZKT should be allocated to the claimers
        uint256 amountForGasSubsidy; // this is how much ZKT was minted for the gas subsidy
        uint256 numberOfEligibleClaimers; // how many submitters are eligible for claiming, used to determine the reward amount
        uint256 amountClaimedByClaimers; // keep track of how much ZKT has been claimed by the claimers, primarily used to expire unclaimed rewards
    }
    ...
    function initialize(address _esZKTAddress, address _zktAddress, address _gasSubsidyAddress, uint256 gasSubsidyPercentage_) public initializer {
        ...
        esZKTAddress = _esZKTAddress;
        zktAddress = _zktAddress;
    }

    function getCombinedTotalSupply() public view returns (uint256) {
        return  esZKT(esZKTAddress).totalSupply() + ZKT(zktAddress).totalSupply() + _allocatedTokens;
    }
     ...
     */
    function setGuardianNodeLicenseAddress(address _nodeLicenseAddress) external onlyRole(DEFAULT_ADMIN_ROLE) {
        guardianNodeLicenseAddress = _nodeLicenseAddress;
    ...
    function calculateChallengeEmissionAndTier() public view returns (uint256, uint256) {
        ...
        uint256 maxSupply = ZKT(zktAddress).MAX_SUPPLY();
    ...
    function submitChallenge(
        ...
        // mint zkt for the gas subsidy
        ZKT(zktAddress).mint(gasSubsidyRecipient, amountForGasSubsidy);
        ...
        // add the amount that will be given to claimers to the allocated field variable amount, so we can track how much esZKT is owed
    ...
    function claimReward(
        ...
        esZKT(esZKTAddress).mint(owner, reward);
        ...
        // unallocate the tokens that have now been converted to esZKT
