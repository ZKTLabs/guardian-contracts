...
const func: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
  ...
  const stub = (await ethers.getContract(
    "ComplianceRegistryStub_L1"
  )) as ComplianceRegistryStub_L1;
  ...
  for (let i = 1; i < 3; i++) {
    ...
    console.log(
      `BlacklistComplianceRegistry_${i} deployed at ${deployedResult.address}`
    );
    const deployedContract = await ethers.getContractAt(
      `ComplianceRegistry`,
      deployedResult.address
    );
    const tx0 = await deployedContract.grantRole(
      await deployedContract.COMPLIANCE_REGISTRY_STUB_ROLE(),
      stubAddress
    );
    await tx0.wait();
...
func.dependencies = [
  "DeployNetworkSupportedRegistry",
  "DeployComplianceRegistryStub_L1",
];
export default func;
